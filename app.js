...
const elements = {
    ...
    btnShareClean: document.getElementById('btnShareClean')
...
// --- FUNCIÓN COMPATIBLE PARA COPIAR AL PORTAPAPELES (MÓVILES Y ORDENADORES) ---

function copyToClipboard(text) {

    return new Promise((resolve, reject) => {

        // 1. Intentar el método moderno de Clipboard API (solo HTTPS / localhost)
        ...
        if (navigator.clipboard && window.isSecureContext) {
            ...
            // 2. Método de respaldo (Fallback) usando un área de texto temporal

            // Esto es crucial para móviles en redes locales (HTTP) o navegadores antiguos
            ...
            try {
                ...
                // Evitamos scroll o saltos visuales al añadir el elemento al DOM
                ...
                // Selección especial para dispositivos iOS (iPhone/iPad)
                ...
                if (successful) {
                    ...
                    reject(new Error("execCommand('copy') devolvió falso"));
...
// --- COMPARTIR ENLACE COMPLETO (CON DATOS) ---
if (elements.btnShareConfig) {
    elements.btnShareConfig.onclick = () => {
        const url = window.location.href;
        copyToClipboard(url).then(() => {
            const originalText = elements.btnShareConfig.innerHTML;
            elements.btnShareConfig.innerHTML = "¡Copiado! 📋";
            elements.btnShareConfig.style.background = "#22c55e"; // Verde feedback
            setTimeout(() => {
                elements.btnShareConfig.innerHTML = originalText;
                elements.btnShareConfig.style.background = ""; // Restaura CSS
            }, 2000);
        }).catch((err) => {
...
// --- COMPARTIR ENLACE LIMPIO (SIN DATOS) ---
if (elements.btnShareClean) {
    elements.btnShareClean.onclick = () => {
        const cleanURL = window.location.protocol + "//" + window.location.host + window.location.pathname;
        copyToClipboard(cleanURL).then(() => {
            const originalText = elements.btnShareClean.innerHTML;
            elements.btnShareClean.innerHTML = "¡Copiado! 📋";
            elements.btnShareClean.style.background = "#22c55e"; // Verde feedback
            setTimeout(() => {
                elements.btnShareClean.innerHTML = originalText;
                elements.btnShareClean.style.background = ""; // Restaura CSS
            }, 2000);
        }).catch((err) => {
            console.error("Error al copiar al portapapeles:", err);
            alert("No se pudo copiar automáticamente. Cópialo de la barra del navegador.");
